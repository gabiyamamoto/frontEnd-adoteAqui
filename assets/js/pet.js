    ...
    async function carregarPet(id) {
        try {
            ...
            console.error(err);
            document.getElementById("pet-info").innerHTML = "<p>Pet não encontrado</p>";
        }
    }
    ...
    function renderizarPet(pet) {
        ...
        if (pet.adotado) {
            ...
            botaoHtml = `<button class="btn-adotar" onclick="contatarDono('${pet.nome}', '${pet.telefone}')">Quero Adotar 💙</button>`;
        }
    ...
    function contatarDono(nome, telefone) {
        const telDestino = telefone || "5511999999999"; 
        const msg = encodeURIComponent(`Olá! Vi o pet ${nome} no site Adote Aqui e tenho interesse!`);
